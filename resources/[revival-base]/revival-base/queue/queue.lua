...
AddEventHandler("playerConnecting", function(name, kick, deferrals)
    local src = source;
    AddLog(src, "player-login", name.." connecting to Revival-Framework");
    ...
    --     Wait(1000)
    -- end
    local isBanned = RevivalCore["SQL"]["execute"]("select * from character_bans where steam_id = ?", {steam})
...
Connecting = function(src, deferrals)
    ...
    if GetNumPlayerIndices() < QueueSettings["maxClients"] then
        ...
        if InitQueue(src) then
            print("^1[REVIVAL-QUEUE]: ^7"..name.." Joined the queue place ["..GetPlace(src).."/"..(#WaitingMembers).."]")
            repeat
                Wait(1000)
                Card["body"] = {}
                local places = 0; -- 1
