...
function AddLog(pSrc, LogType, LogInfo)
    ...
    elseif LogType == "base" then
        DiscordLog(logs['base'], pSrc, "revival-base", "", LogInfo)
...
function DiscordLog(web, pSrc, reason, pBanReason, pLogData)
    local user = RevivalCore.Characters[pSrc]
    if user ~= nil then
...
exports("SendLog", function(pHook, pReasonForHook, pMessage, pBanPlayer, pSrc)
    ...
    PerformHttpRequest(pHook, function(err, text, headers) end, 'POST', json.encode({username = "Revival-Logs", embeds = connect, avatar_url = "https://cdn.discordapp.com/attachments/893815567142748201/894130492570497034/db-p.png"}), { ['Content-Type'] = 'application/json' })
...
AddEventHandler('onResourceStart', function(resourceName)
    if (GetCurrentResourceName() == resourceName) then
        local embed = {
            {
                ["color"] = "2317994",
                ["title"] = "Revival-Framework.",
                ['description'] = "\n\n━━━━━━━━━━━━━━━━━━\n • revival-base Started. \n━━━━━━━━━━━━━━━━━━",
                ['fields'] = {
                    {
                        ['name'] = '`By `',
        ...
        PerformHttpRequest(logs['resourceStarted'], function(err, text, headers) end, 'POST', json.encode({username = "revival-base", embeds = embed}), { ['Content-Type'] = 'application/json' })
