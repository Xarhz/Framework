...
RevivalCore.CurrentRequestId 		= 0
...
RevivalCore.GetCharacter = function()
	if RevivalCore.LocalPlayer ~= nil then
		return RevivalCore.LocalPlayer
	end
	return nil
end
...
RevivalCore.SpawnVehicle = function(model, cb, coords, isnetworked)
    local model = (type(model)=="number" and model or GetHashKey(model))
    local coords = coords ~= nil and coords or RevivalCore.GetCoords(PlayerPedId())
	...
	TriggerEvent("revival-parking:addKeys",veh,GetVehicleNumberPlateText(veh))
    if cb ~= nil then
        cb(veh)
    end
end
...
exports('SpawnVehicle', function(model, cb, coords, isnetworked)
	RevivalCore.SpawnVehicle(model, cb, coords, isnetworked)
end)


RevivalCore.DeleteVehicle = function(vehicle)
    SetEntityAsMissionEntity(vehicle, false, true)
    DeleteVehicle(vehicle)
end

RevivalCore.GetVehicleInDirection = function()
...
RevivalCore.GetClosestVehicle = function(source)
...
RevivalCore.TriggerServerCallback = function(name, cb, ...)
	RevivalCore.ServerCallbacks[RevivalCore.CurrentRequestId] = cb

	TriggerServerEvent('revival-base:triggerServerCallback', name, RevivalCore.CurrentRequestId, ...)

	if RevivalCore.CurrentRequestId < 65535 then
		RevivalCore.CurrentRequestId = RevivalCore.CurrentRequestId + 1
	else
		RevivalCore.CurrentRequestId = 0
	end
end
...
RevivalCore.SpawnLocalObject = function(model, coords, cb)
	local model = (type(model) == 'number' and model or GetHashKey(model))
...
RevivalCore.DeleteObject = function(object)
	SetEntityAsMissionEntity(object, false, true)
	DeleteObject(object)
end

RevivalCore.GetClosestPlayer = function(coords)
	local players         = RevivalCore.GetCharacters()
...
RevivalCore.GetCoords = function(entity)
...
RevivalCore.RequestAnimDict = function(animDict, cb)
...
function FetchVehProps(vehicle)	
  ...
  return {	
    ...
    fuelLevel         = exports["reviva-fuel"]:GetFuel(vehicle),	
...
function SetVehProps(vehicle, props)	
  ...
  if props.fuelLevel ~= nil then
    exports["reviva-fuel"]:SetFuel(vehicle, props.fuelLevel + 0.0)
...
RegisterNetEvent('revival-base:serverCallback')
AddEventHandler('revival-base:serverCallback', function(requestId, ...)
	RevivalCore.ServerCallbacks[requestId](...)
	RevivalCore.ServerCallbacks[requestId] = nil
end)