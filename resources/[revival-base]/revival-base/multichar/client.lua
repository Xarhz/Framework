...
RegisterNetEvent('revival-multicharacters:setupCharacters')
AddEventHandler('revival-multicharacters:setupCharacters', function(data)
    SendNUIMessage(
        {
            ["action"] = "setup";
            ["chars"] = data;
...
RegisterNetEvent('revival-multicharacters:init')
AddEventHandler('revival-multicharacters:init', function()
    SetNuiFocus(false, false)
...
RegisterNUICallback('createPed', function(fuck)
    ...
    resetPed()
    Wait(100);
    TriggerServerEvent("revival-multicharacters:GetCharacter", cid)
    TriggerServerEvent("revival-multicharacters:pedshit", cid)
    for i=1,25 do
...
RegisterNUICallback('PlayerList', function(fuck)
    TriggerServerEvent('revival-multicharacters:GetPlayers')
end)

RegisterNetEvent("revival-multicharacters:GetPlayers")
AddEventHandler("revival-multicharacters:GetPlayers", function(id, players, max)
...
RegisterNetEvent("revival-multicharacters:getPed", function(data)
    if ped ~= nil then
...
RegisterNetEvent("revival-multicharacters:GetCharacter", function(data)
    SendNUIMessage(
        {
            ["action"] = "select";
            ["chars"] = data;
...
RegisterNUICallback('selectCharacter', function(data)
    ...
    resetPed()
    Character_close()
    TriggerServerEvent('revival-multicharacters:charSelect', cid, false)
    SetNuiFocus(false, false);
...
RegisterNUICallback('deleteCharacter', function(data)
    local cid = tonumber(data.cid)
    resetPed()
    TriggerServerEvent('revival-multicharacters:deleteChar', cid)
    Wait(300)
end)
...
RegisterNUICallback('createCharacter', function(data)
    ...
    resetPed()
    TriggerServerEvent('revival-multicharacters:createCharacter', charData)
    Wait(300)
    TriggerServerEvent('revival-multicharacters:charSelect', charData.cid, true)
    isFirst = true
    Character_close()
    SetNuiFocus(false, false);
    setClothingForChar()
...
function setClothingForChar()
    ...
    local gender = RevivalCore["LocalPlayer"]["sex"]
    if gender ~= "Male" then
        SetSkin(GetHashKey("mp_f_freemode_01"), true)
    else
        SetSkin(GetHashKey("mp_m_freemode_01"), true)
    end
    TriggerEvent("revival-clothing:openClothing")
    TriggerEvent("revival-clothing:inSpawn", true)
    SetEntityHeading(PlayerPedId(),64.71)
    SetGameplayCamRelativeHeading(180.0)
    SetGameplayCamRelativePitch(4.0, 1.0)
end

RegisterNetEvent("revival-multicharacters:finishedClothing")
AddEventHandler("revival-multicharacters:finishedClothing", function(endType)
    local playerped = PlayerPedId()
    local playerCoords = GetEntityCoords(playerped)
    local pos = vector3(-3965.88,2014.55, 501.6)
    local distance = #(playerCoords - pos)

    TriggerEvent("revival-clothing:inSpawn", false)
    ...
    if distance <= 10 then
    	...
    	if endType == "Finished" then
            ...
            Wait(450)
    		spawnChar()
            TriggerServerEvent("revival-multicharacters:licenses") -- todo add licenses
    	else
    		InitSession(true)
    	end
    end	
...
function spawnChar()
    deleteTrain()
    TriggerEvent("revival-base:initial")
    TriggerServerEvent('revival-multicharacters:visiable')
end
...
local isNear = false
Citizen.CreateThread(function()
	exports["revival-polyzone"]:AddBoxZone("spawnZone", vector3(-3962.55, 2014.91, 500.91), 31.2, 20.2,  {
		name="spawnZone",
        heading=340,
        minZ=498.51,
        maxZ=506.31
...
AddEventHandler("revival-polyzone:enter", function(name)
    if name ~= "spawnZone" then return end
    checkPlayers()
end)
AddEventHandler("revival-polyzone:exit", function(name)
    if name ~= "spawnZone" then return end
    listening = false
end)
