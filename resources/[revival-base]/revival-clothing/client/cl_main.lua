...
local inSpawn = false
AddEventHandler("revival-clothing:inSpawn", function(pInSpawn)
    inSpawn = pInSpawn
end)
...
RegisterNUICallback('escape', function(data, cb)
    ...
    if (newCharacter) then
        ...
        -- TriggerEvent('revival-clothing:enable', true)
    end
    cb('ok')
end)
...
function Save(save, close, newFadeStyle)
    if save then
        ...
        if not startingMenu or passedClothing then
            TriggerServerEvent("revival-clothing:insert_character_current", data)
            TriggerServerEvent("revival-clothing:insert_character_face", data)
            TriggerServerEvent("revival-clothing:set_tats", currentTats)
            TriggerEvent("revival-multicharacters:finishedClothing","Finished")
        ...
        TriggerEvent("revival-multicharacters:finishedClothing","Old")
        LoadPed(oldPed)
    end
    ...
    TriggerEvent('revival-weapons:getAmmo')
    CustomCamera('torso',true)
    -- TriggerEvent("e-blips:updateAfterPedChange",exports["revival_exports"]:GetData("myjob"))
    startingMenu = false
end
...
RegisterNetEvent('revival-clothing:saveCharacterClothes')
AddEventHandler('revival-clothing:saveCharacterClothes', function()
...
RegisterNetEvent("revival-clothing:inService")
AddEventHandler("revival-clothing:inService", function(service)
    isService = service
end)

RegisterNetEvent("revival-clothing:hasEnough")
AddEventHandler("revival-clothing:hasEnough", function(menu)
...
RegisterNetEvent("revival-clothing:setclothes")
AddEventHandler("revival-clothing:setclothes", function(data, alreadyExist)
  player = PlayerPedId()
  ...
  Citizen.Wait(500)
  TriggerEvent("facewear:update")
  TriggerServerEvent("revival-clothing:get_character_face", nil, data.fadeStyle)
  TriggerServerEvent("revival-clothing:retrieve_tats")
  TriggerServerEvent("hud:SetMeta")
  TriggerEvent("Animation:Set:Reset")
...
RegisterNetEvent('revival-clothing:newCharacterSpawned')
AddEventHandler('revival-clothing:newCharacterSpawned', function(sex)
    if not exports["revival-inventory"]:hasEnoughOfItem("idcard",1,false) then
        TriggerEvent("player:receiveItem","idcard",1,true)
    end
    if not exports["revival-inventory"]:hasEnoughOfItem("mobilephone",1,false)then
...
RegisterNetEvent("revival-clothing:nitzan", function()
    SetSkin("mp_f_freemode_01", true)
    local LocalPlayer = exports["revival-base"]:GetCharacter()
...
RegisterNetEvent("revival-clothing:SetDefaultClothes")
AddEventHandler("revival-clothing:SetDefaultClothes", function(shit)
    if shit == "Male" then
        SetSkin("mp_m_freemode_01", true)
    else
        SetSkin("mp_f_freemode_01", true)
...
RegisterNetEvent("revival-clothing:AdminSetModel")
AddEventHandler("revival-clothing:AdminSetModel", function(model)
...
RegisterNetEvent("revival-clothing:defaultReset")
AddEventHandler("revival-clothing:defaultReset", function()
    local LocalPlayer = exports["revival-base"]:GetCharacter("LocalPlayer")
    local gender = LocalPlayer.sex
    Citizen.Wait(1000)
    if gender ~= "Male" then
        SetSkin(`mp_f_freemode_01`, true)
...
RegisterNetEvent("revival-clothing:settattoos")
AddEventHandler("revival-clothing:settattoos", function(playerTattoosList)
    currentTats = playerTattoosList
    SetTats(GetTats())
end)

RegisterNetEvent("revival-clothing:setpedfeatures")
AddEventHandler("revival-clothing:setpedfeatures", function(data)
    player = PlayerPedId()
    if data == false then
...
RegisterNetEvent('revival-clothing:outfits')
AddEventHandler('revival-clothing:outfits', function(pAction, pId, pName)
    if pAction == 1 then
        TriggerServerEvent("revival-clothing:set_outfit",pId, pName, GetCurrentPed())
    elseif pAction == 2 then
        TriggerServerEvent("revival-clothing:remove_outfit",pId)
    elseif pAction == 3 then 
        TriggerEvent('item:deleteClothesDna')
        TriggerEvent('db-sounds:client:PlayOnOne','Clothes1', 0.6)
        TriggerServerEvent("revival-clothing:get_outfit", pId)
    else
        TriggerEvent("hud:save")
        TriggerServerEvent("revival-clothing:list_outfits")
    end
end)

RegisterNetEvent('revival-clothing:FUCKOFFHEREBRO')
AddEventHandler('revival-clothing:FUCKOFFHEREBRO', function(skincheck)
    local menuData = {}
    local takenSlots = {}
...
RegisterNetEvent("revival-clothing:use:outfit")
AddEventHandler("revival-clothing:use:outfit", function(data)
    TriggerServerEvent("revival-clothing:get_outfit", data[2])
end)

RegisterNetEvent("revival-clothing:remove:outfit")
AddEventHandler("revival-clothing:remove:outfit", function(data)
    TriggerServerEvent("revival-clothing:remove_outfit", data[2])
end)

RegisterNetEvent("revival-clothing:return", function()
    TriggerEvent("hud:save")
    TriggerServerEvent("revival-clothing:list_outfits")
end)
...
RegisterUICallback("db-ui:clothing:deleteOutfit", function(data, cb)
    ...
    TriggerEvent('revival-clothing:remove:outfit', { true, data.key })
end)

RegisterUICallback("db-ui:clothing:changeOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
    TriggerEvent('revival-clothing:use:outfit', { true, data.key })
end)

RegisterNetEvent('revival-clothing:outfit')
AddEventHandler('revival-clothing:outfit', function(args,sentType)
    if sentType == 1 then
        local id = args[2]
        TriggerEvent("revival-clothing:outfits", sentType, id)
...
RegisterUICallback("db-ui:clothing:addOutfit", function(data, cb)
    ...
    TriggerServerEvent("revival-clothing:set_outfit", outfitSlot, outfitName, GetCurrentPed())
end)

RegisterNetEvent('revival-clothing:enable')
AddEventHandler('revival-clothing:enable', function(status)
    actionDress = status
end)
...
RegisterCommand("outfitadd", function(source, args, rawCommand)
    if actionDress == true then
        if args[1] and args[2] then
            TriggerEvent('revival-clothing:outfits', 1, args[1], args[2])
        else
...
RegisterCommand("outfits", function(source, args, rawCommand)
    if actionDress == true then
        TriggerEvent('revival-clothing:outfits', 4)
    else
        TriggerEvent('notification', "You are not near a closet",2)
    end
end, false)

RegisterNetEvent("revival-clothing:list:outfits")
AddEventHandler("revival-clothing:list:outfits", function(data)
    TriggerServerEvent("revival-clothing:options:oufits", data.slot)
end)


RegisterNetEvent("revival-clothing:use:outfit")
AddEventHandler("revival-clothing:use:outfit", function(data)
    TriggerServerEvent("revival-clothing:get_outfit", data.slot)
end)

RegisterNetEvent("revival-clothing:remove:outfit")
AddEventHandler("revival-clothing:remove:outfit", function(data)
    TriggerServerEvent("revival-clothing:remove_outfit", data.slot)
end)
...
RegisterNetEvent("revival-clothing:attempt:change", function(data)
    ...
    if pNewName then
        TriggerServerEvent('revival-clothing:attempt:change',  data.slot, pNewName[1].input)
    else
...
RegisterCommand("m1", function(source, args, rawCommand)
    if exports["revival-inventory"]:hasEnoughOfItem("mask",1) then
...
RegisterNetEvent('revival-clothing:openClothing')
AddEventHandler('revival-clothing:openClothing', function(pDontShowBarber, pShouldCost, pStartingMenu)
...
RegisterNetEvent('revival-clothing:openBarber', function(pShouldCost)
    if pShouldCost ~= nil and pShouldCost then
...
AddEventHandler("revival-polyzone:enter", function(zone, data)
...
AddEventHandler("revival-polyzone:exit", function(zone)
    local currentZone = MenuData[zone]
    if currentZone then
        listening = false
        TriggerEvent("revival-clothing:enable", false)
        exports["db-ui"]:hideInteraction()
    end
end)
...
AddEventHandler("revival-inventory:itemUsed", function(item)
...
AddEventHandler("revival-inventory:itemUsed", function(pItem, pInfo)
  ...
  if not hasCgChainOn then
    ...
    Citizen.CreateThread(function()
      while hasCgChainOn do
        ...
        if info.diamond then
          local gemInfo = exports['revival-clothing']:getGemData(info.diamond)
    ...
    Citizen.CreateThread(function()
      while hasCgChainOn do
        Citizen.Wait(math.random(10000, 120000))
        if info.ruby then
          local gemInfo = exports['revival-clothing']:getGemData(info.ruby)
    ...
    Citizen.CreateThread(function()
      while hasCgChainOn do
        ...
        if info.tanzanite then
          local gemInfo = exports['revival-clothing']:getGemData(info.tanzanite)
...
RegisterNetEvent("revival-inventory:itemCheck", function(item)
  local model = chainModels[item]
  if not model then return end
  local qty = exports["revival-inventory"]:hasEnoughOfItem(item, 1)
...
AddEventHandler("revival-clothing:openCGChainCrafting", function()
  if not hasChainAccess() then return end
  TriggerEvent("server-inventory-open", "42087", "Craft")
end)
AddEventHandler("revival-clothing:infuseChainWithGems", function()
  if not hasChainAccess() then return end
  local cid = exports["revival_exports"]:GetData("citizenid")
  RPC.execute("revival-clothing:infuseCGChain", cid)
end)
