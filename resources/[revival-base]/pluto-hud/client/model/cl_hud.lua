...
RegisterNUICallback('pluto-hud:hudSetPreferences', function(data, cb)
    ...
    TriggerEvent("pluto-preferences:setPreferences", data)
end)

AddEventHandler('pluto-preferences:setPreferences', function(pData)
    --exports["pluto-interface"]:sendAppEvent('preferences', pData)
end)
...
AddEventHandler("pluto-hud:watch", function()
...
AddEventHandler("pluto-hud:watch:reset", function()
    showCompassFromWatch = false
    toggleCompass()
end)

RegisterNetEvent('pluto-inventory:itemCheck')
AddEventHandler('pluto-inventory:itemCheck', function (item, hasItem, quantity)
...
Citizen.CreateThread( function()
  ...
  Citizen.CreateThread(function ()
	while true do
       ...
       -- print("pRadioActive", pRadioActive)
...
RegisterNetEvent('pluto-boosting:client:setBoostCompletions')
AddEventHandler('pluto-boosting:client:setBoostCompletions', function (pAmount)
...
function getFuel()
    fuel = 15--exports["pluto-vehicles"]:CurrentFuel() or 50
end
    ...
    Citizen.CreateThread(function()
        while engineOn do
            ...
            if GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
                harnessDurability = 0--exports["pluto-vehicles"]:GetVehicleMetadata(veh, 'harness')
            end
...
-- RegisterNetEvent('pluto-hud:show_weapon_out')
-- AddEventHandler('pluto-hud:show_weapon_out', function()
--     sendAppEvent("hud", {
--         isArmed = true
--     })
-- end)

-- RegisterNetEvent('pluto-hud:hide_weapon_out')
-- AddEventHandler('pluto-hud:hide_weapon_out', function()
--     sendAppEvent("hud", {
--         isArmed = false
--     })
-- end)
...
RegisterNetEvent('pluto-weapons:client:showWeaponFireRate')
AddEventHandler('pluto-weapons:client:showWeaponFireRate', function (pToggle)
...
function getVehicleInDirection(coordFrom, coordTo)
  ...
  RegisterNetEvent("pluto-hud:jerry_can:refuel")
  AddEventHandler("pluto-hud:jerry_can:refuel",function()
      vehicleCurrentFuel = exports["pluto-vehicles"]:CurrentFuel(veh) --DecorGetInt(veh, "GetVehicleCurrentFuel")
      if exports['np-inventory']:hasEnoughOfItem('883325847', 1) then
        ...
        if finished == 100 then
            if exports['pluto-inventory']:hasEnoughOfItem('883325847', 1) then
                --DecorSetInt(veh, "GetVehicleCurrentFuel", 100)
                exports["pluto-vehicles"]:SetVehicleFuel(veh, 100)
                TriggerEvent('inventory:removeItem', '883325847', 1)
            else
                TriggerEvent('DoLongHudText', 'Try again!', 2)
            end
    
            local vehicleCurrentFuel = exports["pluto-vehicles"]:CurrentFuel(veh) --DecorGetInt(veh, "GetVehicleCurrentFuel")
