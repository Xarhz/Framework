...
local CharacterData = {}
RegisterNetEvent('revival-base:characterSpawned', function()
    isOnline = true
    CharacterData = exports['revival-base']:GetCharacter()
    if CharacterData['job']['name'] == 'police' or 'ems' then
        TriggerServerEvent('revival_exports:DoctorCount')
        TriggerServerEvent('revival_exports:OfficerCount')
    end
end)

RegisterNetEvent('revival-base:updateJob', function(Job)
    CharacterData.job = Job
    TriggerServerEvent('revival_exports:DoctorCount')
    TriggerServerEvent('revival_exports:OfficerCount')
end)

RegisterNetEvent('revival-base:updateSecondaryJob', function(SecondaryJob)
    CharacterData.secondaryJob = SecondaryJob
end)

RegisterNetEvent('revival-base:updateDuty', function(Duty)
...
RegisterNetEvent('revival-base:updateCash', function(Cash)
	CharacterData.cash = Cash
end)

RegisterNetEvent('revival-base:updateBank', function(Bank)
	CharacterData.bank = Bank
end)

RegisterNetEvent('revival-base:updateGang', function(Gang)
    CharacterData.gang = Gang
    fivem(CharacterData.gang)
end)
local blip;
RegisterNetEvent('revival-base:updateLifeStyle', function(LifeStyle)
...
RegisterNetEvent('revival-base:updateRank', function(Rank)
	CharacterData.rank = Rank
end)

DoctorCount = 0
RegisterNetEvent('revival_exports:SetDoctorCount', function(amount)
    DoctorCount = amount
end)

OfficerCount = 0
RegisterNetEvent('revival_exports:SetOfficerCount', function(amount)
    OfficerCount = amount
end)
...
function money(amount, action, account)
    TriggerServerEvent('revival_exports:money',amount, action, account)
end
...
RegisterNetEvent('revival_exports:incall')
AddEventHandler('revival_exports:incall', function(status)
  incall = status
end)
...
RegisterNetEvent("revival_exports:intrunk")
AddEventHandler("revival_exports:intrunk", function(trunksent)
  intrunk = trunksent
end)